...
the durable document before launch. Do not retry creation. The sole idempotent
exception is a structured, non-retryable error that the exact requested item
already exists: resolve that exact item once, verify its identity and context,
and continue only when its requested metadata and work requirements are
compatible. Never mutate an existing item to manufacture compatibility. Other
creation failures end setup and send a native Herdr failure notification,
without focusing or typing into another pane.
