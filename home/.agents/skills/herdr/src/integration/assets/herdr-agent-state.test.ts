...
const socketPlugins = [
	...
	{
		name: "Kilo",
		modulePath: "./kilo/herdr-agent-state.js",
...
test("OpenCode stays disabled without the Herdr socket environment", async () => {
	...
	const { HerdrAgentStatePlugin } = await importFresh(
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
	...
	handlers.get("agent_start")?.({}, context)
	await waitFor(() => requestStates(requests).length === 2)
	eventHandlers.get("herdr:blocked")?.(
...
test("Pi reports the session replacement source", async () => {
	...
	const reportedSession = () =>
		requests.find(
			(request) =>
				isRecord(request) && request.method === "pane.report_agent_session",
		)
	const deadline = Date.now() + 1_000
	...
	expect(
		isRecord(request) && isRecord(request.params)
			? request.params.session_start_source
			: null,
	).toBe("new")
})
...
test("Pi waits for a replacement session report before publishing state", async () => {
	const recordingSocketPath = join(
		tmpdir(),
		`herdr-pi-session-order-${process.pid}.sock`,
	)
	socketPath = recordingSocketPath
	await rm(recordingSocketPath, { force: true })
	...
	expect(
		requests.map((request) => (isRecord(request) ? request.method : undefined)),
	).toEqual(["pane.report_agent_session", "pane.report_agent"])
})
...
test("Oh My Pi retries working before a queued idle state", async () => {
	const { attemptedRequests } =
		await startDroppedFirstResponseServer("omp-retry")
