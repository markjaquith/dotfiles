...
const previewDocsSourceDir = resolve(
	repoRoot,
...
const generatedVersionsDataPath = resolve(
	repoRoot,
	"website/src/data/docs-versions.json",
)
...
async function preparePublicAssets() {
	...
	for (const directory of ["assets", "css", "agent-detection"]) {
		await cp(
			resolve(repoRoot, "website", directory),
			resolve(publicDir, directory),
			{
				recursive: true,
			},
		)
...
async function prepareDocs() {
	...
	await copyPreparedDocs(
		previewDocsSourceDir,
		previewDocsDir,
		(content, relativePath) => rewritePreviewDocContent(content, relativePath),
	)
	...
	for (const entry of manifest.versions) {
		...
		const snapshotDocsRoot = resolve(
			versionsDir,
			version,
		...
		try {
			configReferences[version] = JSON.parse(
...
async function copyPreparedDocs(
	sourceDir,
	destinationDir,
	rewrite,
	pathPrefix = "",
...
async function collectDocsScope(sourceDir, excludedDirectories = new Set()) {
	...
	async function walk(directory, prefix = "") {
		for (const entry of await readdir(directory, { withFileTypes: true })) {
			...
			const locale =
				segments[0] === "ja" || segments[0] === "zh-cn"
					? segments.shift()
					: "root"
			const page = segments
				.join("/")
...
export function rewriteVersionDocContent(
	content,
	{ version, tag, sourceRoot, relativePath },
) {
	...
	const indexPrefix =
		relativePath === "index.mdx"
			? content.replace(
					...
					"title: Herdr next documentation",
				)
			: content
