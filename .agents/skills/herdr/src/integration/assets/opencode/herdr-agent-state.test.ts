...
async function loadPlugin() {
	importCounter += 1
	const { HerdrAgentStatePlugin } = await import(
		`./herdr-agent-state.js?test=${importCounter}`
	)
...
test("suppresses redundant same-session updates", async () => {
	...
	await plugin.event({
		event: {
			type: "session.updated",
			properties: { sessionID: "replacement-session" },
		},
	})
...
test("reports child prompts without replacing the root session", async () => {
	...
	for (const type of ["permission.asked", "question.asked"]) {
		await plugin.event({
			event: { type, properties: { sessionID: "child-session" } },
		})
	}
	for (const type of [
		"permission.replied",
