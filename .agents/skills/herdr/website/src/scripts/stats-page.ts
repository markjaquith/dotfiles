...
function weekStartOf(day: string): string {
	...
	return new Date(date.getTime() - sinceMonday * 86_400_000)
		.toISOString()
		.slice(0, 10)
}
...
): { partial: boolean; daysIn: number } {
	if (!lastDataDay) return { partial: false, daysIn: 7 }
	const weekEnd = new Date(dayEpoch(weekStart) + 6 * 86_400_000)
		.toISOString()
		.slice(0, 10)
	const partial = lastDataDay < weekEnd || weekEnd >= todayUtc()
	const daysIn = partial
		? Math.min(daysBetween(weekStart, lastDataDay) + 1, 7)
		: 7
...
function deltasFromCumulative(daily: SeriesPoint[]): SeriesPoint[] {
	...
	for (let index = 1; index < daily.length; index += 1) {
		deltas.push({
			d: daily[index].d,
			v: Math.max(daily[index].v - daily[index - 1].v, 0),
		})
	}
	return deltas
}
...
function lineChart(options: LineChartOptions): string {
	...
	const xFor = (day: string): number =>
		margin.left + ((dayEpoch(day) - x0) / (x1 - x0)) * innerW
	const yFor = (value: number): number =>
		margin.top + innerH - (value / yMax) * innerH
	...
	const grid = ticks
		.map((tick) => {
			...
			return (
				`<line class="grid" x1="${margin.left}" y1="${y}" x2="${width - margin.right}" y2="${y}"/>` +
				`<text class="tick" x="${margin.left - 8}" y="${y}" dy="0.32em" text-anchor="end">${fmtCompact(tick)}</text>`
			)
		})
		.join("")

	const monthLabels = monthStartsBetween(
		points[0].d,
	...
	const eventBaseX =
		Math.min(...events.map((event) => event.dotX), Infinity) - 12
...
function barChart(options: BarChartOptions): string {
	...
	const grid = ticks
		...
		.map((tick) => {
			const y = yFor(tick).toFixed(1)
			return (
				`<line class="grid" x1="${margin.left}" y1="${y}" x2="${BAR_W - margin.right}" y2="${y}"/>` +
	...
	const maxValue = Math.max(
		...weeks.filter((week) => !week.partial).map((week) => week.value),
		0,
	)
	...
	const bars = weeks
		.map((week, index) => {
			...
			const capLabel =
				!week.partial && week.value === maxValue && week.value > 0
...
function cumulativeTable(points: SeriesPoint[], unit: string): string {
	...
	const rows = [...byMonth.values()]
		.map(
			(point) =>
				`<tr><td>${fmtDay(point.d)}</td><td>${fmtInt(point.v)}</td></tr>`,
		)
		.join("")
...
function timelineTerminal(stats: StatsJson, milestones: Milestone[]): string {
	...
	for (const entry of stats.timeline) {
		...
		lines.push({
			day: entry.d,
			text: escapeHtml(entry.label),
			note: escapeHtml(note),
		})
	}
	let previousDay =
		stats.timeline.find((entry) => entry.label.includes("first star"))?.d ??
		stats.bornDay
	for (const milestone of milestones) {
		lines.push({
			day: milestone.day,
			text: `★ ${fmtInt(milestone.value)}`,
			note: `+${daysBetween(previousDay, milestone.day)} days`,
		})
...
function attachChartInteractions(root: ParentNode): void {
	root.querySelectorAll<HTMLElement>(".chart").forEach((chart) => {
		const dataEl = chart.querySelector<HTMLScriptElement>(
		...
		fetch(REPO_API_URL)
			...
			.then((repo) => {
				if (repo && typeof repo.stargazers_count === "number")
					animateTo(repo.stargazers_count)
			})
			.catch(() => {
...
function render(stats: StatsJson): void {
	...
	const last7d = (() => {
		...
		if (!last) return null
		const weekAgoDay = new Date(dayEpoch(last.d) - 7 * 86_400_000)
			.toISOString()
			.slice(0, 10)
		const anchor = [...starsDaily]
			.reverse()
			.find((point) => point.d <= weekAgoDay)
		return anchor ? stats.stars.total - anchor.v : null
	})()
	if (last7d !== null)
		setText("hero-note", `+${fmtInt(last7d)} stars in the last 7 days`)
	...
	const lastFullWeek = [...downloadsWeekly]
		.reverse()
		.find((week) => !week.partial)
	...
	const visitorsAvg = lastSevenTraffic.length
		? Math.round(
				lastSevenTraffic.reduce((sum, point) => sum + point.v, 0) /
					lastSevenTraffic.length,
			)
		: null
	...
	setText(
		"kpi-age-sub",
		`born ${fmtDay(stats.bornDay)}, ${stats.bornDay.slice(0, 4)}`,
	)
	...
	setHtml(
		"chart-stars-week",
		barChart({
			weeks: weeklySums(starsDeltas, lastStarDay),
			colorVar: "--accent",
			unit: "stars",
	...
	setHtml(
		...
		barChart({
			weeks: downloadsWeekly,
			colorVar: "--green",
			unit: "downloads",
		}) + weeklyTable(downloadsWeekly, "downloads"),
	)
	const visitorsWeekly = weeklyAverages(
		trafficDaily,
		trafficDaily.at(-1)?.d ?? null,
	)
	setHtml(
		"chart-visitors-week",
		barChart({
			weeks: visitorsWeekly,
			colorVar: "--yellow",
			unit: "avg daily visitors",
	...
	setText(
		"traffic-sub",
		`herdr.dev traffic over the last ${stats.traffic.windowDays} days, measured by Cloudflare.`,
	)
	setHtml(
		"rank-countries",
		rankList(
			stats.traffic.topCountries.map((row) => ({
				label: `${flagEmoji(row.code)} ${countryName(row.code)}`.trim(),
				value: row.visits,
			})),
			"visits",
		),
	...
	setHtml(
		"rank-referrers",
		rankList(
			stats.traffic.topReferrers.map((row) => ({
				label: row.host,
				value: row.visits,
			})),
			"visits",
