...
const stableReferencePath = resolve(
	repoRoot,
...
export function normalizeVersion(value) {
	...
	if (!match)
		throw new Error(`version must look like 0.7.5 or v0.7.5, got ${value}`)
	return match[1]
}
...
async function extractGitTree(ref, sourceRoot, destinationRoot) {
	...
	if (files.length === 0)
		throw new Error(`${ref}:${sourceRoot} contains no files`)
...
async function readManifest() {
	try {
		...
		if (error.code === "ENOENT")
			return { schema_version: 1, current: null, versions: [] }
		throw error
	}
}
...
async function snapshotTag(tag, sourceRoot) {
	...
	await extractGitTree(
		tag,
		sourceRoot,
		resolve(destination, "website/src/content/docs"),
	)
	...
	if (gitPathExists(tag, referenceSource)) {
		const referenceDestination = resolve(
			destination,
			"website/src/data/config-reference.json",
		)
		...
		await writeFile(
			...
			git(["show", `${tag}:${referenceSource}`], { binary: true }),
		)
	}
...
export async function backfillVersions() {
	...
	const entries = new Map(
		manifest.versions.map((entry) => [entry.version, entry]),
	)

	for (const tag of tags) {
		const version = normalizeVersion(tag)
		if (entries.has(version) || !gitPathExists(tag, "website/src/content/docs"))
			continue
		const nextRoot = "docs/next/website/src/content/docs"
		const sourceRoot =
			gitPathExists(tag, nextRoot) &&
			!gitTreesEqual(tag, "website/src/content/docs", nextRoot)
				? nextRoot
	...
	const current = JSON.parse(
		await readFile(resolve(repoRoot, "website/latest.json"), "utf8"),
	).version
	await writeManifest({
		schema_version: 1,
...
export async function checkVersions() {
	...
	if (manifest.current !== normalizeVersion(latest)) {
		throw new Error(
			`docs current version ${manifest.current} does not match website latest ${latest}`,
		)
	}

	const expectedOrder = sortVersionsNewestFirst(manifest.versions).map(
		({ version }) => version,
	)
	if (
		JSON.stringify(manifest.versions.map(({ version }) => version)) !==
	...
	for (const entry of manifest.versions) {
		if (seen.has(entry.version))
			throw new Error(`duplicate docs version ${entry.version}`)
		seen.add(entry.version)
		if (normalizeVersion(entry.tag) !== entry.version) {
			throw new Error(
				`docs version ${entry.version} has mismatched tag ${entry.tag}`,
			)
		}
		if (
			![
			...
			throw new Error(
				`docs version ${entry.version} has unsupported source ${entry.source}`,
			)
		}
		...
		const expected = listGitFiles(entry.tag, sourceRoot)
			.map((path) => relative(sourceRoot, path))
			.sort()
		const actual = (await listFiles(snapshotRoot)).sort()
		if (JSON.stringify(actual) !== JSON.stringify(expected)) {
			throw new Error(
				`documentation file list for ${entry.version} differs from ${entry.tag}:${sourceRoot}`,
			)
		}
		for (const relativePath of expected) {
			...
			const taggedContent = git(["show", `${entry.tag}:${taggedPath}`], {
				binary: true,
			})
			if (!actualContent.equals(taggedContent)) {
				throw new Error(
					`${entry.version}/${relativePath} differs from ${entry.tag}:${sourceRoot}`,
				)
			}
		}
		...
		if (gitPathExists(entry.tag, referenceSource)) {
			...
			const taggedReference = git(["show", `${entry.tag}:${referenceSource}`], {
				binary: true,
			})
			if (!actualReference.equals(taggedReference)) {
				throw new Error(
					`${entry.version} config reference differs from ${entry.tag}:${referenceSource}`,
				)
			}
		} else {
			try {
				...
				throw new Error(
					`${entry.version} has an unexpected config reference snapshot`,
				)
			} catch (error) {
	...
	process.stdout.write(
		`validated ${manifest.versions.length} documentation snapshots\n`,
	)
}
...
export async function publishVersion(tag) {
	...
	const manifest = await readManifest()
	const existing = manifest.versions.find(
		(entry) => entry.version === release.version,
	)
	if (existing && existing.tag !== release.tag) {
		throw new Error(
			`version ${release.version} is already associated with ${existing.tag}`,
		)
	}
	...
	await replaceDirectory(
		resolve(snapshotRoot, "src/content/docs"),
		stableDocsDir,
	)

	const snapshotReference = resolve(
		snapshotRoot,
	...
	if (gitPathExists(tag, nextReadme)) {
		await writeFile(
			...
			git(["show", `${tag}:${nextReadme}`], { binary: true }),
		)
	}
...
async function main() {
	...
	throw new Error(
		"usage: node website/scripts/docs-versions.mjs backfill | check | publish <tag>",
	)
}
