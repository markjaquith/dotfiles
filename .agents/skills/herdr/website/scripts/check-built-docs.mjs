...
const nonCanonicalDocsUrl =
	/https:\/\/herdr\.dev\/(?:ja\/|zh-cn\/)?docs\/(?:preview|\d+\.\d+\.\d+)(?:\/|<)/
...
const preview = await readFile(
	resolve(distDir, "docs/preview/index.html"),
	"utf8",
)
...
assertIncludes(
	stable,
	'data-pagefind-filter="version[content]" content="stable"',
)
...
assertIncludes(
	preview,
	'data-pagefind-filter="version[content]" content="preview"',
)
assertIncludes(preview, 'name="robots" content="noindex, nofollow"')
assertIncludes(
	archived,
	`data-pagefind-filter="version[content]" content="${versions.current}"`,
)
...
if (archived.includes(`This page documents Herdr ${versions.current}`)) {
	throw new Error(
		"the current immutable snapshot must not be labeled as outdated",
	)
}
const versionSelect = stable.match(
	/<select[^>]*aria-label="Documentation version"[^>]*>([\s\S]*?)<\/select>/,
)?.[1]
if (!versionSelect)
	throw new Error("stable docs are missing the version selector")
...
const previous = versions.versions.find(
	(entry) => entry.version !== versions.current,
)
if (previous) {
	...
	assertIncludes(
		previousArchive,
		`This page documents Herdr ${previous.version}`,
	)
}
...
if (nonCanonicalDocsUrl.test(sitemap)) {
	throw new Error(
		"preview or immutable documentation URLs must not appear in the sitemap",
	)
}
...
if (build.count > 20_000) {
	throw new Error(
		`website build has ${build.count} files, exceeding the Cloudflare Pages free-plan limit`,
	)
}
if (build.largest.bytes > 25 * 1024 * 1024) {
	throw new Error(
		`website asset ${build.largest.path} is ${build.largest.bytes} bytes, exceeding Cloudflare Pages' 25 MiB limit`,
	)
}
process.stdout.write(
	`validated ${versions.versions.length} documentation versions in ${build.count} website files\n`,
)

function assertIncludes(content, expected) {
	if (!content.includes(expected))
		throw new Error(`built documentation is missing ${expected}`)
}
...
async function inspectFiles(directory, root = directory) {
	...
	for (const entry of await readdir(directory, { withFileTypes: true })) {
		...
		if (entry.isDirectory()) {
			...
			if (nested.largest.bytes > result.largest.bytes)
				result.largest = nested.largest
