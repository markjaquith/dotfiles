	...
	try {
		...
		return {
			...
			prs: Array.isArray(data.prs)
				...
				: [],
			fetchedAt:
				typeof data.fetchedAt === "string" ? data.fetchedAt : undefined,
		}
	} catch {
...
function upsertIssue(data: CachedData, issue: IssueInfo): void {
	data.issues = [
		issue,
		...data.issues.filter((item) => item.number !== issue.number),
	]
}
...
function shouldRefresh(data: CachedData): boolean {
	...
	return (
		...
		Date.now() - fetchedAt > CACHE_REFRESH_INTERVAL_MS
	)
}
...
function filterPRs(prs: PRInfo[], token: string): PRInfo[] {
	...
	return fuzzyFilter(
		...
		query,
		(pr) =>
			`${pr.number} ${pr.title} ${pr.headRefName} ${pr.state} ${pr.author}`,
	).slice(0, MAX_SUGGESTIONS)
}
	...
	return {
		async getSuggestions(
			...
			if (issueToken === undefined && prToken === undefined) {
				...
				const prCandidate = extractPrefixCandidate(
					textBeforeCursor,
					`${PR_PREFIX}:`,
				)
			...
			if (issueToken !== undefined) {
				...
				if (
					...
					const fetched = await refreshOptions.fetchIssueByNumber(
						Number(issueToken),
					)
