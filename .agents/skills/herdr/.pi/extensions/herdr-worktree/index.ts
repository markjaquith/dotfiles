...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptSnippet:
			"Create a Herdr worktree workspace and continue the active pi session in it",
		promptGuidelines: [
		...
		parameters: Type.Object({
			...
			closeOldPane: Type.Optional(
				Type.Boolean({
					description:
						"Close the old Herdr pane after the old pi process exits. Defaults to true.",
				}),
			),
...
async function startHerdrWorktree(
	...
	if (options.closeOldPane && !oldPaneId) {
		throw new Error(
			"HERDR_PANE_ID is missing; cannot close the old Herdr pane safely",
		)
	}
	...
	if (!currentFile) {
		throw new Error(
			"Current pi session is not persisted, so it cannot be continued in a worktree",
		)
	}

	const sourceCheckout = await canonicalDirectory(
		options.sourceCheckout || SOURCE_CHECKOUT,
	)
	...
	try {
		...
		if (created.workspaceId) {
			await herdr(
				pi,
				["workspace", "focus", created.workspaceId],
				signal,
				sourceCheckout,
				10_000,
			)
...
async function createHerdrWorktree(
	...
	if (!worktreePath || !rootPaneId) {
		throw new Error(
			"Herdr worktree create response did not include worktree.path and root_pane.pane_id",
		)
	}
...
async function runInNewPane(
	...
	const command = ["pi", "--session", sessionFile, continuation]
		.map(shellQuote)
		.join(" ")
...
async function scheduleOldPaneCleanup(
	...
	const result = await pi.exec("sh", ["-lc", launcher], {
		signal,
		timeout: 5_000,
	})
	if (result.code !== 0) {
		throw new Error(
			`Failed to schedule old pane cleanup: ${result.stderr || result.stdout}`,
		)
	}
}
...
async function herdrJson(
	...
	try {
		...
		throw new Error(
			`Herdr returned non-JSON output for ${args.join(" ")}: ${raw}`,
		)
	}
	if (json.error) {
		throw new Error(
...
function parseCommandArgs(args: string): StartOptions {
	...
	for (let i = 0; i < tokens.length; i += 1) {
		...
		if (token === "--branch") options.branch = requireValue(tokens, ++i, token)
			...
			options.label = requireValue(tokens, ++i, token)
		else if (token === "--source")
			options.sourceCheckout = requireValue(tokens, ++i, token)
	...
	options.sourceCheckout =
		cleanOptional(options.sourceCheckout) ?? SOURCE_CHECKOUT
	return options
}
