...
For pre-_ hooks, commands in a table run sequentially. For post-_ hooks, they run concurrently in the background. Post-\* hooks that need ordering guarantees can use [pipeline ordering](#pipeline-ordering).
...
Pre-_ hooks ignore pipeline structure — all commands run serially regardless, since pre-_ hooks are blocking by nature.
