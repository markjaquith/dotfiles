...
extract_urls() {
  local url opens closes lefts rights

  # The path class stays permissive so URLs that legitimately contain () or []
  # are kept intact.
...
open_detected_urls() {
	local urls count selected action

	urls="$1"

	if [[ -z "$urls" ]]; then
		echo "No URLs detected"
		if [[ -n "${URL_CHOOSER_WAIT_ON_EMPTY:-}" ]]; then
	...
	if [[ "$count" -eq 1 ]]; then
		...
		selected=$(echo "$urls" | fzf --ansi --expect=y --prompt='URL> ')
		action="${selected%%$'\n'*}"
		selected="${selected#*$'\n'}"

		case "$action" in
			y)
				[[ -n "$selected" ]] && print -rn -- "$selected" | pbcopy
				;;
			*)
...
open_tmux_urls() {
	...
	raw=$(tmux capture-pane -J -p)
	urls=$(print -r -- "$raw" | extract_urls)
	open_detected_urls "$urls"
}

open_herdr_urls() {
	local herdr pane raw urls lines

	herdr="${HERDR_BIN_PATH:-herdr}"
	pane="${HERDR_URL_CHOOSER_TARGET_PANE:-}"

	if [[ -z "$pane" && -n "${HERDR_PLUGIN_CONTEXT_JSON:-}" ]] && command -v jq >/dev/null 2>&1; then
		pane=$(jq -r '.focused_pane_id // .pane_id // .pane.pane_id // empty' <<<"$HERDR_PLUGIN_CONTEXT_JSON")
	fi
	...
	if [[ -z "$pane" ]]; then
		if ! command -v jq >/dev/null 2>&1; then
			echo "jq is required to find the current Herdr pane"
			sleep 1
			return 1
		fi
