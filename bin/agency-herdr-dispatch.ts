...
${allowWorkingDependencies ? "The dispatcher received the explicit --allow-working-dependencies opt-in. Pass it to the helper ONLY if the original user request explicitly asks to start now despite existing working dependencies; otherwise stop and report the authorization mismatch." : "No working-dependency override was authorized by the dispatcher. Do not add --allow-working-dependencies. If the user needs that exception, report the blocker for an explicitly opted-in dispatch."}
