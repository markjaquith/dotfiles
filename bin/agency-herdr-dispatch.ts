...
1. If creation was requested, create the durable item directly with the appropriate noninteractive Agency mutation and --json. Do not add --work or --auto to creation. Explicit new/separate/follow-up work means a distinct item, not reuse of the investigation. Otherwise resolve the existing requested item. Capture its exact absolute document path from successful Agency JSON output. Do not retry creation after any failure. A structured non-retryable Agency error whose code and message say that the exact requested task, phase (including parent task), or epic ID already exists is the only idempotent creation exception: resolve that exact item once with the matching ${agencyCommand} task show '<id>' --json, ${agencyCommand} phase show '<task-id>' '<phase-id>' --json, or ${agencyCommand} epic show '<id>' --json command. Require the show result's IDs, parent, and canonical path plus the context target kind to match the intended item. Then verify it with ${agencyCommand} context '<absolute-document-path>' --json and require any explicitly requested repo, branch, base, references, and work requirements to be compatible with the existing durable item. Never update, recreate, reopen, remove, or otherwise mutate the existing item merely to make it compatible. If the error is not that exact already-exists case, lookup fails, identity is ambiguous, or existing metadata conflicts, follow the pre-helper failure procedure below and stop.
